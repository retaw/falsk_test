...
######################game logic#####################
    ...
    def addAgency(self, agencyid, password, superviorid, superviorIsAdmin):
        if Redishandler.me().playerExisit(agencyid) is not True:
            return u"ID={}的玩家不存在".format(agencyid)
        cursor = self.getCursor()
        try:
            if superviorIsAdmin is not True:
    ...
    def queryAgencyIncomeDetail(self, agencyid):
        ...
        try:
            ...
            rowData = []
            for row in cursor:
                rowData.append((row[0], row[1], row[2], row[3].strftime("%Y-%m-%d %H:%M:%S")))
                totalMoney += row[1]
            return True, rowData, totalMoney
        except MySQLdb.Error as e:
            logger.error(e)
            return False, e
